// ui_for_user_list/lib/upload_files_main.dart
import 'package:flutter/material.dart';
import 'package:http/http.dart' as http;
import 'package:shared_preferences/shared_preferences.dart';
import 'package:ui_for_user_list/main_login.dart';
import 'dart:convert';
import 'models/uploaded_file.dart'; // Import the NEW UploadedFile model
...
import 'package:ui_for_user_list/models/uploaded_file_details_screen.dart';
class UploadedFileManagementScreen extends StatefulWidget {
  const UploadedFileManagementScreen({super.key});

  @override
  State<UploadedFileManagementScreen> createState() =>
      _UploadedFileManagementScreenState();
}

class _UploadedFileManagementScreenState
    extends State<UploadedFileManagementScreen> {
  bool _isDocumentManagementExpanded = true; // Keep expanded for initial view

  // TextEditingControllers for each search field (updated for UploadedFile)
  ...
  final TextEditingController _parentIdSearchController =
      TextEditingController();
  final TextEditingController _refIdSearchController = TextEditingController();
  final TextEditingController _moduleIdSearchController =
      TextEditingController();
  ...
  List<UploadedFile> _displayedUploadedFiles =
      []; // Change to List<UploadedFile>
  ...
  // Method to fetch data from your Flask API for Uploaded Files
  Future<void> _fetchUploadedFilesFromApi() async {
    setState(() {
      _isLoading = true;
      _errorMessage = null;
    });
  ...
  if (authToken == null) {
    ...
    // Replace with your actual API URL for uploaded files
    const String apiUrl =
        'http://127.0.0.1:5000/admin/documents/list'; // Adjust this URL
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          'type': _typeSearchController.text,
          'parent_id': _parentIdSearchController.text,
          'ref_id': _refIdSearchController.text,
          'module_id': _moduleIdSearchController.text,
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> uploadedFileDataList = responseData['data'];
        final List<UploadedFile> fetchedUploadedFiles = uploadedFileDataList
            .map((json) => UploadedFile.fromJson(json))
            .toList();

        setState(() {
          _displayedUploadedFiles = fetchedUploadedFiles;
        ...
        setState(() {
          _errorMessage =
              'Failed to load uploaded files: ${errorData['error'] ?? response.statusCode}';
          _isLoading = false;
          _displayedUploadedFiles = []; // Clear on error
          _totalItems = 0;
          _totalPages = 0;
        });
      }
  ...
  void _applySearchFilters() {
    ...
    _fetchUploadedFilesFromApi(); // Now calls the API
  }
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (_isLoading)
                        ...
                        Expanded(
                          child:
                              _buildUploadedFilesTable(), // Changed method call
                        ),
                      _buildPagination(),
                      _buildCopyright(),
                    ],
  ...
  // --- Build Uploaded Files Table ---
  Widget _buildUploadedFilesTable() {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Padding(
            ...
            child: Text(
              '$_totalItems Uploaded Files', // Display total count from API
              style: TextStyle(
                fontSize: 16,
              ...
              child: SingleChildScrollView(
                ...
                child: Container(
                  ...
                  child: DataTable(
                    ...
                    rows: _displayedUploadedFiles.map((uploadedFile) {
                      // Use UploadedFile object properties
                      return DataRow(
                        cells: [
                          DataCell(Text(uploadedFile.id?.toString() ?? 'N/A')),
                          DataCell(
                            Text(uploadedFile.envId?.toString() ?? 'N/A'),
                          ),
                          DataCell(Text(uploadedFile.type ?? 'N/A')),
                          DataCell(Text(uploadedFile.parentId ?? 'N/A')),
                          DataCell(Text(uploadedFile.refId ?? 'N/A')),
                          DataCell(
                            Text(uploadedFile.moduleId?.toString() ?? 'N/A'),
                          ),
                          DataCell(Text(uploadedFile.status ?? 'N/A')),
                          DataCell(
                            Text(
  ...
  Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => UploadedFileDetailsPage(
          fileId: uploadedFile.id!, // Pass the uploaded file's ID
        ),
      ),
    );
                                  },
