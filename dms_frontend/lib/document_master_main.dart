// ui_for_user_list/lib/document_master_main.dart
import 'package:flutter/material.dart';
import 'package:http/http.dart' as http;
import 'package:ui_for_user_list/main_login.dart';
import 'dart:convert';
import 'models/document_master.dart'; // Import the NEW DocumentMaster model
...
import 'package:ui_for_user_list/models/document_master_details_screen.dart'; // Import user details page

class DocumentMasterManagementScreen extends StatefulWidget {
  const DocumentMasterManagementScreen({super.key});

  @override
  State<DocumentMasterManagementScreen> createState() =>
      _DocumentMasterManagementScreenState();
}

class _DocumentMasterManagementScreenState
    extends State<DocumentMasterManagementScreen> {
  bool _isDocumentManagementExpanded = true; // Keep expanded for initial view

  // TextEditingControllers for each search field (updated for DocumentMaster)
...
Future<String?> getAuthToken() async {
  ...
  // Method to fetch data from your Flask API for Document Master
  Future<void> _fetchDocumentMasterFromApi() async {
    setState(() {
      _isLoading = true;
      _errorMessage = null;
    });
  ...
  if (authToken == null) {
    ...
    // Replace with your actual API URL for document master
    // Assuming a new API endpoint for document master, e.g., on port 5003
    const String apiUrl =
        'http://127.0.0.1:5000/admin/document_master/list'; // <--- ADJUST THIS URL/PORT
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          // Ensure these keys match what your Flask API expects for Document Master
      ...
      if (response.statusCode == 200) {
        ...
        // Parse the list of document master entries
        final List<dynamic> documentDataList = responseData['data'];
        final List<DocumentMaster> fetchedDocuments = documentDataList
            .map((json) => DocumentMaster.fromJson(json))
            .toList();
        ...
        setState(() {
          _errorMessage =
              'Failed to load document master: ${errorData['error'] ?? response.statusCode}';
          _isLoading = false;
          _displayedDocuments = []; // Clear on error
          _totalItems = 0;
          _totalPages = 0;
        });
      }
  ...
  void _applySearchFilters() {
    ...
    _fetchDocumentMasterFromApi(); // Now calls the API
  }
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (_isLoading)
                        ...
                        Expanded(
                          child:
                              _buildDocumentMasterTable(), // Changed method call
                        ),
                      _buildPagination(),
                      _buildCopyright(),
                    ],
  ...
  Widget _buildDocumentMasterTable() {
      ...
      child: Column(
        ...
        children: [
          Padding(
            ...
            child: Text(
              '$_totalItems Document Master Entries', // Display total count from API
              style: TextStyle(
                fontSize: 16,
                ...
                child: Container(
                  ...
                  child: DataTable(
                    ...
                    rows: _displayedDocuments.map((document) {
                      // Use DocumentMaster object properties
                      return DataRow(
                        cells: [
                          ...
                          DataCell(
                            Text(document.moduleId?.toString() ?? 'N/A'),
                          ),
                          DataCell(Text(document.status ?? 'N/A')),
                          DataCell(
                            Text(
                              document.createdAt?.toLocal().toString().split(
                                        ' ',
                                      )[0] ??
                                  'N/A',
                            ),
                          ...
                          DataCell(
                            Text(
                              document.updatedAt?.toLocal().toString().split(
                                        ' ',
                                      )[0] ??
                                  'N/A',
                            ),
    ...
    MaterialPageRoute(
      builder: (context) => DocumentMasterDetailsPage(documentMasterId: document.id!), // Corrected for Document Master
    ),
  );
                                  },
                                ),
